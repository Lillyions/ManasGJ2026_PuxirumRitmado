...
namespace Dypsloom.RhythmTimeline.Scoring
{
    ...
    public class ScoreManager : MonoBehaviour
    {
        ...
        protected virtual void HandleOnNoteTriggerEvent(NoteTriggerEventData noteTriggerEventData)
        {
            ...
            switch (m_AccuracySpawnPointOption) {
                ...
                case AccuracySpawnPointOption.OnMouse:
                    var mousePosition = CrossPlatformInput.MousePosition;
        ...
        {
            var maxScore = m_CurrentSong.RhythmClipCount * 800; // based on the Example Score Settings Accuracy Table Perfect Value
            float sucessPercentage = (GetScore() / maxScore) * 100f;

            if (sucessPercentage <= 20)
                return 1;
            else if (sucessPercentage > 20 && sucessPercentage <= 30)
                return 2;
            else if (sucessPercentage > 30 && sucessPercentage <= 50)
                return 3;
            else if (sucessPercentage > 50 && sucessPercentage <= 70)
                return 4;
            else if (sucessPercentage > 70)
                return 5;
            else
                return 0;
        }
