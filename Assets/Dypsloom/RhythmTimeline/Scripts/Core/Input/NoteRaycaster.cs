namespace Dypsloom.RhythmTimeline.Core.Input
{
    ...
    public class NoteRaycaster : MonoBehaviour
    {
        ...
        public void Update()
        {
            ...
            var inputUp = CrossPlatformInput.GetMouseButtonUp();
            var inputDown = CrossPlatformInput.GetMouseButtonDown();

            if (!m_UseMouseButtonDownUp || (!inputUp && !inputDown)) { return; }

            if ((!m_UsePhysics2D || !RaycastNote2D(CrossPlatformInput.MousePosition, out note)) &&
                (!m_UsePhysics3D || !RaycastNote3D(CrossPlatformInput.MousePosition, out note))) { return; }
