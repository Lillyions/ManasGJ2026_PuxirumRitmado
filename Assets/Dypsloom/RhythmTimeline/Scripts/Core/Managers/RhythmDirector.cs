...
namespace Dypsloom.RhythmTimeline.Core.Managers
{
    ...
    public class RhythmDirector : MonoBehaviour
    {
        ...
        [Space]
        [Tooltip(
            "For local multiplayer you may have multiple Rhythm tracks playing, match the PlayerIds for all relevant components.")]
        ...
        public void PlaySong(RhythmTimelineAsset songTimeLine)
        {
            if (songTimeLine == null) {
                Debug.LogError("The song timeline cannot be null", gameObject);
                return;
            }
        ...
        protected virtual void SetUpAudioTrackBinding(AudioTrack audioTrack)
        {
            ...
            if (m_AudioTracksCount >= m_AudioSources.Length) {
                Debug.LogError(
                    "The Audio Source could not be found for Audio Track at index " + m_AudioTracksCount +
                    " Please assign another audio source in the inspector.", gameObject);
            } else {
        ...
        protected virtual void HandleSongEnded(PlayableDirector playableDirector)
        {
            EndSong();
        }

        public virtual void EndSong()
        {
        ...
        {
            if (m_TrackObjects == null) {
                return;
            }

            // Clear the active notes on the track objects.
            for (int i = 0; i < m_TrackObjects.Length; i++) {
                m_TrackObjects[i].ClearNotes();
            }
        }
        ...
        public void RegisterRhythmClips(int trackID, List<RhythmClip> trackRhythmClips)
        {
            ...
            foreach (var rhythmClip in trackRhythmClips) {
                if (m_AllRhythmClips.Contains(rhythmClip)) {
                    continue;
                }
